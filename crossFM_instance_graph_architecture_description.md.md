...
| $\alpha_v, \alpha_t$ | Learnable residual blend scalars (visual, text) |
...
$$
\mathcal{N}_k(i) = \operatorname{argtop\text{-}k}_j \; S_{ij}
$$

$$
\tilde{S}_{ij} = \begin{cases} S_{ij} & \text{if } j \in \mathcal{N}_k(i) \;\text{ or }\; i \in \mathcal{N}_k(j) \\ 0 & \text{otherwise} \end{cases}
$$
...
where $\tilde{\mathbf{D}}_{ii} = \sum_j \tilde{A}_{ij}$. This produces the adjacencies $\hat{\mathbf{A}}_{\text{VV}} \in \mathbb{R}^{M \times M}$ and $\hat{\mathbf{A}}_{\text{TT}} \in \mathbb{R}^{C \times C}$.
...
The messages are combined with learnable gate scalars $g_{\text{cv}}, g_{\text{ct}}, g_{\text{xv}}, g_{\text{xt}} \in \mathbb{R}$ (initialised to 0, so $\sigma(g) = 0.5$ at the start):
...
The graph refinement is blended with the raw features using two learnable scalars $\alpha_v, \alpha_t \in \mathbb{R}$ (initialised via $\sigma^{-1}(0.5) = 0$):
...
|-----------|-----------|-------|
| $\mathbf{W}_{\text{VV}}, \mathbf{W}_{\text{TT}}, \mathbf{W}_{\text{TV}}, \mathbf{W}_{\text{VT}}, \mathbf{W}_{\text{xV}}, \mathbf{W}_{\text{xT}}$ | $6 \times d^2$ | 1,572,864 |
| $\text{LayerNorm}_V, \text{LayerNorm}_T$ | $2 \times 2d$ (weight + bias) | 2,048 |
| CFIG gate scalars | $g_{\text{cv}}, g_{\text{ct}}, g_{\text{xv}}, g_{\text{xt}}$ | 4 |
| Model scalars | $\alpha_v, \alpha_t, \gamma, \tau$ | 4 |
| **Total** | | **1,574,920** |
